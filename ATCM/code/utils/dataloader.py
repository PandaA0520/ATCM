...
class MyCollator(object):
    ...
    def __call__(self, batch):
        ...
        if sents_aug2 is not None: 
            # further add stochastic synoym replacement augmentation
          #  for sent in sents_aug2:
             #   if isinstance(sent, str):
                   # augmented_sent = naw.SynonymAug(aug_src='wordnet', aug_p=0.05).augment(sent)[0]
                  #  sents_aug2.append(augmented_sent)
               # else:
                # 对于非字符串类型的数据，可以选择跳过或进行适当的处理
                  #  print(f"Skipping or handling non-string data: {sent}")
...
def get_dataloader(data_path, n_labeled_per_class, bs, load_mode='semi'):
    ...
    #train_df = pd.read_excel(os.path.join(data_path,'train.xlsx'), engine='openpyxl')
    #dev_df = pd.read_excel(os.path.join(data_path,'dev.xlsx'), engine='openpyxl')
